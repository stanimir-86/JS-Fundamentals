function heroesOfCodeAndLogic(arr) {
    function creatHero(heroName, heroHp, heroMana) {
        let hero = {
            name: heroName,
            hp: heroHp,
            mana: heroMana,
    ...
    for (let i = 0; i < numberOfHeroes; i++) {
        ...
        let hero = creatHero(name, hp, mana);
        map.set(name, hero);
    ...
    for (let i = 0; i < commandsArr.length; i++) {
        ...
        let name = tokens[1];
        let values = tokens.slice(2);//така си вземам всички елементи сле индекс 1 (Solmyr - 50)
